...
@defSelfJointOperator 에서 $A$가 에르미트 연산자이려면 아래가 성립해야 한다.
$
  A = overline(A)^TT
$

이때 $overline(A)^TT$를 $A$의 켤레 전치(conjugate transpose) 또는 에르미트 전치(Hermitian transpose)라고 한다.
...
자기 수반 연산자의 성질은 차원의 크기에 관련이 없었지만, 우리가 본 스펙트럼 정리는 유한차원에서만 성립한다는 것이다. 이 강의에서는 유한 차원에 대한 것만 필요하기 때문이다. 하지만 무한차원으로 일반화하여 확장한 스펙트럼 정리도 존재한다. 증명에 대한 것 등 자세한 것은 선형대수학에서 공부하자.
...
#axiom(title: [양자역학적 측정의 공리])[
  각 측정 실험에 대해 우리는 양자상태의 공간 $V$ 상의 자기 수반 연산자인 관측가능량 $A$를 할당할 수 있다. 스펙트럼 정리에 의해, $A$는 고유공간을 갖는다. 
