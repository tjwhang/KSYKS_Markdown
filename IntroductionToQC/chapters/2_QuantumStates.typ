...
#axiom(title: [양자 상태])[
  각 양자계에는 그에 대응하는 상태 공간이 존재한다. 양자계의 한 상태는 이 상태 공간 상의 벡터이다. 상태 공간은 복소수체 $CC$ 위에서 정의되는 벡터 공간이며, 에르미트 내적이 주어져야 한다. 물리적으로 유의미한 양자 상태는 에르미트 내적에 따른 크기가 1인 단위 벡터이다.
]
...
두 전자기파는 시간에 따라 공간을 통과해 진행하고 있다. 따라서 시간에 따라 $z$ 축 위의 점을 바꿔보며 두 편광 상태의 합을 구해보면, 상태 공간에서 편광 상테 벡터가 반시계 방향으로 회전하는 것을 알 수 있다. 이런 상태를 원형 편광(circular polarization)이라고 한다. 이때, $e^(i pi/2) = i$이므로 같은 상태를 이렇게도 표현할 수 있다.
$
  kat + i kar
$
...
#definition(title: [고윳값과 고유벡터])[
  임의의 $n times n$ 행렬 $A$에 대하여 다음 식에 대해 0이 아닌 해 $vbu(x)$가 존재하면 스칼라 $lambda$는 행렬 $A$의 고윳값(eigenvalue)이라고 한다.
  $
    A vbu(x) = lambda vbu(x)
  $
  이때 해가 되는 벡터 $vbu(x)$는 고윳값 $lambda$에 대응하는 고유벡터(eigenvector)이다.
...
스핀은 MRI에 사용된다. MRI는 전자가 아니라 원자핵의 스핀을 이용한다. 원자핵의 스핀보다 여러 양성자와 중성자가 붙어 있는 것이므로 2차원 상에 있는 전자의 스핀보다 큰 공간 상에 있다. 원자핵의 스핀 공간은 유한 차원이지만 어떤 원자냐에 따라 달라지는 것이다. 관측된 스핀값은 $plus.minus 1/2$ 뿐이 아니라, $1/3, 2, 5$ 등등 여러 가지가 나올 수 있다. 
...
우리는 이로서 무한 차원 에르미트 공간과 공간에 직교하는(orthogonal) 기저들을 얻는다. 이제 일반 상태는 기저들의 중첩 또는 선형결합이다. 이것은 푸리에 급수로 나타낼 수 있게 된다. 결론적으로, 일반적인 파동함수는 기저 함수들에 대한 푸리에 급수로 전개될 수 있다.
