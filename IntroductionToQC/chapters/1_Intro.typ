...
10년 뒤인 1975년, 무어는 법칙을 2년 마다 두 배로 증가한다고 수정했다. 열역학 제 2법칙의 등 여러 한계 때문에 증가 속도는 앞으로도 줄어들 예정이다. 오늘날 트랜지스터는 대략 50개의 원자 내외의 두께로 되어 있는데, 이대로 간다면 우리는 곧 원자 크기의 한계에 직면할 것이고, 물리 법칙이 달라지게 된다. 한 개의 원자는 한 뭉탱이의 원자와 전혀 다르게 행동하는데(적어도 그렇게 보이는데), 원자 뭉탱이는 고전역학의 지배를 받는 반면 원자 한 개는 양자역학의 지배를 받는다. 원자가 뭉탱이로 있을 때는, 큰수의 법칙에 따라 양자역학에 의한 확률적 현상들이 평균으로 수렴하여 무시해도 될 정도가 된다. 즉, 계속해서 트랜지스터 개수를 늘리기 위해 크기를 줄인다면 곧 디지털 컴퓨터를 설계할 때도 양자역학적 효과를 고려해야한다는 뜻이다.
...
이때 $ket(psi)$는 $CC^2^n$ 상의 벡터이다. 이것은 $n$-큐비트의 복잡도가 $n$의 지수로 증가함을 보여준다. 즉, $n$ 개의 상호작용하는 전자의 스핀을 기술하기 위해서는 $2^n$ 개의 복소수를 저장해야 한다. $n$이 50만 되어도 그 값은 현재의 컴퓨터로는 범접할 수 없는 크기의 천문학적 숫자가 된다.
