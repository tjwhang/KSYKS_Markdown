...
#let trailing = flatten(ruby[い||かえ|][行|き|帰|り], default-opts).first()
#assert(trailing.at("ruby-segments").len() == 4)
#assert(trailing.at("ruby-segments").last().ruby == "")
...
Trailing empty readings: #ruby[い||かえ|][行|き|帰|り]
