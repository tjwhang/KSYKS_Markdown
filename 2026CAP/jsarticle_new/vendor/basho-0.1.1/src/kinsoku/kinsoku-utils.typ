...
#let is-forbidden-start(token, chars) = {
  ...
  let cached = token.at("forbidden-start", default: none)
  if cached != none { return cached }
  if token.type != "char" { return false }
  if type(token.text) != str { return false }
...
#let is-forbidden-end(token, chars) = {
  if token == none { return false }
  let cached = token.at("forbidden-end", default: none)
  if cached != none { return cached }
  if token.type != "char" { return false }
  if type(token.text) != str { return false }
  chars.contains(token.text)
...
#let is-hanging(token, chars) = {
  if token == none { return false }
  let cached = token.at("hanging-punctuation", default: none)
  if cached != none { return cached }
  if token.type != "char" { return false }
  if type(token.text) != str { return false }
  chars.contains(token.text)
...
#let is-unbreakable-pair(prev, current, chars) = {
  ...
  if type(prev.text) != str or type(current.text) != str { return false }
...
#let is-compressible-punctuation(token, chars) = {
  if token == none { return false }
  let cached = token.at("compressible-punctuation", default: none)
  if cached != none { return cached }
  if token.type != "char" { return false }
  if type(token.text) != str { return false }
  chars.contains(token.text)
...
#let count-justification-points(col) = {
  if col.len() < 2 { return 0 }
  let count = 0
  for i in range(col.len() - 1) {
    let token = col.at(i)
...
#let justify-line(col, available-space, config) = {
  ...
  if count > 0 and available-space > 0pt {
    ...
    let result = ()
    for (i, token) in col.enumerate() {
      if i + 1 < col.len() and token.at("justification-point", default: false) {
...
#let is-valid-line-end(token, forbidden-end-chars) = {
  ...
  if type(token.text) != str { return true }
