...
#let jsarticle-book(
    ...
    set par(
        ...
        spacing: 1em,
        justify: true,
        justification-limits: (spacing: (min: 80%, max: 130%), tracking: (min: -0.01em, max: 0.02em)),
        linebreaks: "optimized",
    )
    show strong: it => text(font: font-gothic, weight: "semibold", it.body)
    ...
    show regex("[\p{scx:Han}\p{scx:Hira}\p{scx:Kana}\p{scx:Hangul}+]"): set text(tracking: -0.06em)
    ...
    show footnote: it => {
        if it.has("label") and it.label == <trans> {
            context {
                ...
                super(size: 0.7em, baseline: -0.3em)[\*#numbering("가", n)]
            }
        } else {
            it
        }
