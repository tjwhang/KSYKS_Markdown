...
#let font-body = (
  ...
  (name: "Minion Pro", covers: regex("[\p{scx:Latn}\p{scx:Grek}\p{scx:Cyrl}+]")),
  ...
  (name: "Batang", covers: regex("[\p{scx:Hangul}+]")),
...
#let font-math = (
  ...
  (name: "STIX Two Math", covers: regex("[∑∏∐∫∬∭∮∯∰⋂⋃⋀⋁]")), // ∫∬∭∮∯∰⋂⋃⋀⋁
  // mod part
  // (name: "Century Old Style", covers: regex("[\p{scx:Latn}\p{scx:Grek}\p{scx:Cyrl}]")),
...
#let cjk-text(it) = {
    ...
    show regex("[\p{scx:Hangul}]++"): it => context {
      ...
      set text(
        baseline: -0.04em, tracking: -0.1em,
        // stroke: 0.005em
      )
      it
    }
...
#let jsarticle-book(
  ...
  show math.equation.where(block: false): it => {
    context {
      let size = measure(it)
      if size.height >= 1em.to-absolute() * 1.1 {
  ...
  // show regex("[\p{scx:Latn}\p{scx:Cyrl}\p{scx:Grek}]+"): set text(size: 0.925em, tracking: -0.03em)
  ...
  show heading: it => {
    ...
    if it.level == 1 {
      if it.has("label") and it.label == <__part__> {
        v(35%)
        align(center)[
        ...
        align(center)[
          #if head-num != none {
            text(size: 1.1em, weight: "medium", font: font-maru, tracking: 0.3em)[#head-num]
            v(1.5em, weak: true)
          }
  ...
  // [수정 후] 소설 모드일 경우 점선(fill)을 none으로 설정합니다.
  ...
  show outline.entry: it => {
    ...
    // [수정 후] 일반 목차 레이아웃 폰트 결정 (소설 모드면 전부 font-body)
    ...
    // [수정 후] 점선(it.fill)이 none일 경우 빈 공간(h(1fr))으로 띄워줌
...
#let jspart(title, num: none) = [
  ...
  #[
    ...
    #align(center)[
      #if num != none [
        #text(size: 1.1em, font: font-body, weight: "medium", tracking: 0.4em, fill: luma(80))[#cjk-text(num)]
        #v(0.2em)
      ]
...
#let jsicover(title, subtitle: none, author: none) = [
  #pagebreak(to: "odd", weak: true)
  #[
    #set page(header: none, footer: none)
    ...
    #v(20%)
    #align(center)[
      #text(size: 3.2em, font: font-maru, weight: "semibold", tracking: 0.1em)[#cjk-text(title)]
      #if subtitle != none [
        #v(1.618em)
        #text(size: 1.2em, font: font-body)[#cjk-text(subtitle)]
      ]
      #if author != none [
        #v(5em)
        #text(size: 1.2em, font: font-body, weight: "medium")[#cjk-text(author)]
      ]
    ]
    #v(1fr)
  ]
