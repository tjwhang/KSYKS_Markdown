...
#let js-default-faces = (
    new-computer-modern: cjk-face("New Computer Modern", optics: (western: (baseline: 0em), punctuation: (baseline: 0em))),
    new-computer-modern-math: cjk-face("New Computer Modern Math"),
    arial: cjk-face("Arial"),
    montserrat: cjk-face("Montserrat"),
    kopub-batang: cjk-face(
        "KoPubWorldBatang_Pro",
        optics: (hangul: (baseline: -0.07em, tracking: -0.02em), punctuation: (baseline: -0.07em)),
    ),
    kopub-dotum: cjk-face(
        "KoPubWorldDotum_Pro",
        optics: (hangul: (baseline: -0.07em, tracking: -0.02em), punctuation: (baseline: -0.07em)),
    ),
    ...
    chosunilbonm: cjk-face("ChosunilboNM", optics: (hangul: (baseline: 0em))),
...
#let _js-composite(western, numbers, hangul, kana, han-ko, han-ja, han-sc, han-tc, punctuation) = cjk-fontset(
    // Chosun contains these glyphs, so ordinary missing-glyph fallback cannot
    // repair their widths. Keep pronunciation letters and combining clusters
    // in the Western face; substitute only standalone stress/length marks.
    ..if western.name == "ChosunilboNM" { ((face: js-default-faces.new-computer-modern, covers: regex("^[ˈˌːˑ]$")),) } else { () },
