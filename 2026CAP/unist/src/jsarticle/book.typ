...
#let _jsarticle-book-render = {
    (
        ...
        horizontal-normalization: (enabled: false, punctuation: true, spaces: true, collapse-punctuation-space: true),
        ...
        let typo = calculate-typography(paper-size, font-size, baseline-ratio, lines-per-page, text-width, cjk-height, cjk-scale, doc-type, cols)
        let single-typo = calculate-typography(paper-size, font-size, baseline-ratio, lines-per-page, text-width, cjk-height, cjk-scale, doc-type, 1)
        ...
        composite-fontsets.insert("ruby", selected-role("ruby", if ruby-family == auto { body-family } else { ruby-family }))
        ...
        let role-font(role) = cjk-fontset-native(composite-fontsets.at(role), common: composite-fontsets.at("common"))
        let role-features(role, base: (:)) = cjk-font-role-features(role, features: base)
        ...
        let default-inline-atom-particles = (ko: (
            "으로",
            "에서",
            "에게",
            "까지",
        ...
        let resolve-page-margins(t) = {
            if page-margin != auto {
                ...
                let resolved = (top: page-margin.top, bottom: page-margin.bottom)
                for key in sides {
                    resolved.insert(key, page-margin.at(key))
                }
                return resolved
            }
        ...
        set page(
            ...
            header: if page-header != auto { page-header } else if doc-type == "novel" { none } else {
                context {
                    ...
                    let has-h1 = query(metadata.where(value: "js-h1-page")).any(marker => marker.location().page() == physical)
                    if here().page-numbering() == none or (not js-running-heads.get() and not has-h1) {
                        ...
                        let header-content = if has-h1 {
                            ...
                            stack(spacing: 0.2em, text(font: role-font("body"), features: role-features("body", base: js-fixed-layout-features))[
                                #if odd [#odd-header-text #h(1fr) #number] else [#number #h(1fr) #chapter-text]
                            ], line(length: 100%, stroke: 0.4pt))
                        }
                        move(dx: -wide-outset, block(width: 100% + 2 * wide-outset, header-content))
                    }
                }
            },
            footer: if page-footer != auto { page-footer } else if doc-type == "article" { none } else {
                context {
                    if here().page-numbering() == none or not js-running-heads.get() {
                        ...
                        align(if odd { right } else { left })[#text(size: 0.9em, fill: luma(65))[#counter(page).display()]]
                    }
                }
            },
        )
        ...
        js-inline-boundary-config.update((gaps: cjk-boundary-gaps, inline-atom-particles: inline-atom-particle-lists))
        ...
        show math.equation: set text(font: fonts.math, weight: "medium", features: js-math-features)
        ...
        set footnote(numbering: (..v) => super(size: 0.7em, baseline: -0.25em, typographic: false, numbering(n => [\*#n], ..v)))
        set footnote.entry(indent: 1.5em, gap: 0.5em, clearance: typo.leading, separator: line(length: 30%, stroke: 0.45pt + luma(30)))
        ...
        set heading(numbering: (..nums) => {
            ...
            } else if (values.at(0) == 0) { values.slice(1).map(str).join(".") } else { values.map(str).join(".") }
        })
        ...
        // The source pass handles nested native containers; the paragraph pass
        // also catches content produced later by contextual components.
        cjk-inline-boundary-spacing(cjk-language-layout(
            layout-content,
            typo.size,
            fontsets: composite-fontsets,
            common-fontset: composite-fontsets.at("common"),
            config: cjk-layout-options,
        ), cjk-boundary-gaps, inline-atom-particles: inline-atom-particle-lists)
    }
}
