...
#let jscjk-inline-boundaries(body) = context {
    ...
    cjk-inline-boundary-spacing(body, config.at("gaps"), inline-atom-particles: config.at("inline-atom-particles"))
}
...
#let _js-horizontal-ruby(reading, base, alignment: auto, dy: 0em) = context {
    ...
    let style-reading = body => cjk-fontset-apply(ruby-fontset, common: common)[#text(top-edge: 0.8 * reading-size, bottom-edge: 0.2 * reading-size)[#body]]
    let render = get-ruby(
        size: reading-size,
        ...
        dy: 0.15 * em - 0.5 * reading-size - offset,
        pos: top,
        alignment: "center",
        delimiter: "|",
        auto-spacing: true,
    ...
    box({
        // Glyph bounds can be zero for Han bases in the active composite font.
        // Rubby's top-anchored overlay would then sit on the base's baseline.
        // Use local em bounds, as for the reading above; keep font routing and
        // the inline baseline intact, including in measured pf-flow fragments.
        set par(leading: 0pt)
...
#let ruby(reading, base, alignment: auto, dy: 0em) = metadata((type: "ruby", text: base, ruby: reading, (_js-ruby-marker): true, alignment: alignment, dy: dy))
...
#let jsvert(body, ..options) = {
    ...
    if flow in ("inline", "region") {
        context cjk-vertical-flow(body, defaults: js-vertical-config.get(), ..options)
    } else {
        metadata((kind: _js-vertical-marker, bodies: (body,) + options.pos(), options: named))
    }
}
#let jscjk-normalize(
    body,
...
#let default-optical-profiles = (ko: (
