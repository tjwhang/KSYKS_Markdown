...
#let has-children(
  ...
  let lo = 0
  // UNIST: bracket the first overflowing prefix before binary refinement.
  // Keep the original zero-based prefix indices, final probes, and splitting
  // semantics. Like upstream bisection, this assumes monotonic prefix fit.
  let hi = 1
  while hi < inner.len() {
    if fits-inside(rebuild(inner.slice(0, hi + 1))) {
      lo = hi
      hi = calc.min(hi * 2, inner.len())
    } else {
      break
    }
  }
