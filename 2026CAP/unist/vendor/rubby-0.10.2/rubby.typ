// Vendored from @preview/rubby:0.10.2.
// Copyright (C) 2023 Andrew Voynov.
// SPDX-License-Identifier: AGPL-3.0-only
//
// Horizontal renderer, with UNIST's empty-segment preservation patch below.
// jsarticle adapts it in src/jsarticle/inline.typ; no vertical-writing logic.

#let _ruby(rt, rb, size, pos, dy, alignment, delimiter, auto-spacing, ruby-style) = {
    ...
    let add-spacing-if-enabled(text) = {
        ...
        // Always add the two spacing slots independently of authored slots.
        // A leading/trailing delimiter may denote an unannotated base segment,
        // e.g. [い||かえ|][行|き|帰|り], and must not be consumed as padding.
    ...
    box(layout(((width, height)) => {
        ...
        while i < rb-array.len() {
            ...
            let bodysize = measure(box(body), width: width, height: height)
            let plain-chars = ruby.clusters().join(h(0pt))
            let rt-plain-width = measure(
                box(par(justify: false, ruby-style(text(size: size, tracking: 0pt, plain-chars)))),
                width: width,
                height: height,
            ).width
            ...
            let rubytext = box(
                ...
                align(
                    if (alignment == "start") { left } else { center },
                    {
                        ...
                        ruby-style(text(size: size, tracking: 0pt, chars))
                    },
                ),
            )
...
#let get-ruby(
    ...
    _ruby(rt, rb, size, pos, dy, alignment, delimiter, auto-spacing, ruby-style)
)
