    ...
    for row in range(rows) {
        ...
        if count > 0 {
            for region in regions.slice(start, start + count).rev() {
                cells.push(
                    box(width: cell-width, height: cell-height, align(right + top, render-page(region, config.layout.gap, config))),
                )
            }
        }
    }
    box(width: block-width, grid(
...
#let page-box(
    ...
    let packed = pack-regions(lines, widths, boundary-gaps, start, rows * columns, cell-width, overhang-threshold)
    ...
    let content = if render {
        box(
            ...
            align(
                center + top,
                render-grid(packed.regions, columns, used-rows, target-width, cell-width, cell-height, column-gap, row-gap, config),
            ),
        )
    } else { none }
...
#let layout-tate(tokens, config, part: "all", initial-height: auto) = {
    ...
    layout(
        size => context {
            ...
            let overhang = measure(h(cfg.layout.at("line-overhang-threshold", default: 0.5em))).width
            let threshold = cfg.layout.at("row-fit-threshold", default: 1.5)
            let target-width = calc.min(available-width, resolve-relative(cfg.layout.at("width", default: auto), available-width, "x"))
            let full-target-height = calc.min(full-height, resolve-relative(cfg.layout.at("height", default: auto), full-height, "y"))
            let first-target-height = calc.min(available-height, resolve-relative(cfg.layout.at("height", default: auto), available-height, "y"))
            let char-box = measure(box(width: cfg.sizing.char-box, height: cfg.sizing.char-box)).height
            if first-target-height < char-box {
                if part == "first" { return [] }
                let full = prepare-layout(tokens, cfg, usable-height: (full-target-height - (rows - 1) * row-gap) / rows)
            ...
            let first-rows = fitted-row-count(first-target-height, full-target-height, rows, row-gap, threshold)
            let first-cell-height = (first-target-height - (first-rows - 1) * row-gap) / first-rows
            let same-geometry = (first-rows == rows and first-target-height >= full-target-height)
            if same-geometry {
            ...
            while consumed < tokens.len() and tokens.at(consumed).type in ("newline", "parbreak",) {
            ...
            let continuation = prepare-layout(tokens.slice(consumed), cfg, usable-height: full-cell-height)
            let lines = paginate(continuation.tokens, full-cell-height, continuation.config, paragraph-start: paragraph-start)
