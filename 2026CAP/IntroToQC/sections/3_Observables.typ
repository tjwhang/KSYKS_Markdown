...
#axiom(title: [양자역학적 측정의 공리])[
    ...
    $
        V = V_lambda_1 plus.o V_lambda_2 plus.o ... plus.o V_lambda_k
    $
...
이 식은 미분방정식이다.
$
    - i hbar pdv(psi, x) = lambda psi \
    pdv(psi, x) = (lambda i)/hbar psi
$

해는 쉽게 구할 수 있다.
$
    psi(x) = C exp(- (lambda i)/hbar x)
$
여기서 $C$는 정규화 상수이다.
...
$
    hat(p) e^(i lambda_k x) = (+ hbar lambda_k) e^(i lambda_k x) \
    hat(p) e^(- i lambda_k x) = (- hbar lambda_k) e^(i lambda_k x)
$

즉 $1/2$의 확률로 운동량 $hbar lambda_k$, $1/2$의 확률로 운동량 $-hbar lambda_k$를 갖는다.
...
#solution[
    ...
    $
        "LHS" =& integral_0^L overline(psi(x)) dot i hbar phi(x) dif x \
        =& cancel(evaluated(i hbar overline(psi) phi)_(x=0)^(x=L)) - integral_0^L i hbar overline(dv(psi, x)) phi(x) dif x \
        because psi(0) = psi(L) = 0
    $
    이고
    $
        "RHS" = & integral_0^L overline(i hbar psi(x)) dot phi(x) dif x \
              = & - integral_0^L hbar overline(dv(psi, x)) phi(x) dif x
    $
...
이제 두 연산자의 곱을 보자.
$
    hat(p) hat(x) (psi) & = - i hbar pdv(, x) [x psi(x)] \
                        & = - i hbar [psi(x) + psi'(x)]
$
또,
$
...
#proof[
    ...
    한편, 여기서 $[Delta hat(A), Delta hat(B)]$는 반 에르미트 연산자, ${Delta hat(A), Delta hat(B)}$는 에르미트 연산자인데,
    $
        expval(Delta hat(A) Delta hat(B))
    ...
    위처럼, 좌변의 첫째 항은 실수가, 둘째 항은 순허수가 나올 것이다. 따라서
    $
          & abs(expval(Delta hat(A) Delta hat(B)))^2 \
