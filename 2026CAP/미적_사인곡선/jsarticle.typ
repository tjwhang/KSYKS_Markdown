...
#let font-math = (
    (name: "STIX Two Math", covers: regex("[*†‡§¶‖]")), //‡§¶‖#*
    (name: "STIX Two Math", covers: regex("[∑∏∐∫∬∭∮∯∰⋂⋃⋀⋁]")), // ∫∬∭∮∯∰⋂⋃⋀⋁
    (name: "Century Old Math Alt", covers: regex("[0-9 A-Za-z𝐴-𝑍𝑎-𝑧Α-Ω𝛼-𝜔𝑨-𝒁𝒂-𝒛 ℏℎ 𝚤𝚥 𝜕]")),
    // mod part
...
#let jsarticle-book(
    ...
    set footnote(numbering: (..v) => box(height: 0.6em, super(
        size: 0.7em,
        baseline: -.3em,
        typographic: true,
        numbering(
        ...
        header: context {
            ...
            let ch-text = if chapters.len() > 0 {
                ...
                if ch.numbering != none {
                    (
                        chapter-format.at(0)
                            + str(counter(heading).at(ch.location()).first())
                            + chapter-format.at(1)
                            + "　"
                            + ch.body
                    )
