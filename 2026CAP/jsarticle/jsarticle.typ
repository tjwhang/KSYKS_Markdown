...
#let font-body = (
  ...
  (name: "New Computer Modern", covers: regex("[\p{scx:Latn}\p{scx:Grek}]")),
  (name: "New Computer Modern", covers: regex("[\p{scx:Cyrl}]")),
...
#let cjk-text(it) = {
  ...
  show regex("[\p{scx:Han}]+"): it => context {
    ...
    set text(tracking: -0.01em, baseline: -0.01em)
    it
  }
...
#let jsarticle-book(
  ...
  // A4/A5에 따른 폰트 크기 및 여백 동적 계산
    let base-font-size = if paper-size == "a5" { 10pt * 1.08 } else { 12pt * 1.08 }

    let margin-inside = if bind == "top" { 27mm } else { 27mm + 5mm }
    let margin-outside = if bind == "top" { 27mm } else { 27mm - 5mm }
    let margin-top = if bind == "top" { 25mm + 15mm } else { 25mm }
    let margin-bottom = 25 * 1.4mm

    if paper-size == "a5" {
      margin-inside = if bind == "top" { 16mm } else { 17mm+2mm }
      margin-outside = if bind == "top" { 16mm } else { 17mm-2mm }
      margin-top = if bind == "top" { 16mm+12mm } else { 16mm }
      margin-bottom = 20mm
    }
    ...
    if type == "novel" {
      ...
      if bind != "top" {
        margin-top = if paper-size == "a5" { 22mm } else { 30mm }
        margin-bottom = if paper-size == "a5" { 28mm } else { 30 * 1.4mm }
        margin-inside = if paper-size == "a5" { 20mm + 2mm } else { 30mm + 5mm }
        margin-outside = if paper-size == "a5" { 20mm - 2mm } else { 30mm - 5mm }
      } else {
        margin-bottom = if paper-size == "a5" { 20mm - 2mm } else { 27mm }
      }
  }
  ...
  set par(
    ...
    justification-limits: (
      ...
      tracking: (min: -0.06em, max: 0.02em),
    ),
  ...
  show math.equation.where(block: true): set block(spacing: 1.4em)
  show math.equation.where(block: true): set par(leading: 0.8em)
