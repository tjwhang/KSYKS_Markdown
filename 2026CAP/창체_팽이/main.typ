...
// --- 0. 문서 메타데이터 ---
#let title = [곡면 상에서 강체의 회전운동]
#let subtitle = "세차운동의 변화와 일반상대론적 블랙홀로의 확장"
#let outline-title = [목차]
#let author-name = "30829 황태준, (30829 정시원)"
#let publisher = "30829 황태준"
#let affiliation = "2026학년도 3학년 창체 탐구보고서"
...
// #heading(numbering: none)[발문]

#heading(numbering: none)[부록]

== 어떻게 했어야 하는가?
(제대로 하려면) 곡면을 먼저 도입하는 것이 아니라, 먼저 아래와 같이 위치벡터를 먼저 놨어야 한다. 다만 대부분의 미분기하학 교재에서도 곡면을 먼저 놓는다.
$
    vb(r)(t) = vec(x(t), y(t), z(t))
$
이렇게 해야 원래처럼 $z$가 곡면 함수 $z = f(x,y)$ 때문에 자연스럽게 독립변수가 되지 않고 시간에 대한 종속변수가 된다. 애초에 모든 것이 $z$가 독립이었기에 자연스럽게 회전축이 법선에 종속되게 된 것이므로, 곡면 구속조건은 식에 내재시킬 것이 아니라 다음과 같이 홀로노믹 제약조건을 별도로 두고
$
    Phi(x, y, z) = z - f(x, y) = 0
$
라그랑주 승수($lambda$) 등을 도입하여 처리했어야 한다. 접점(물체)이 곡선 위에 놓이는 것은 구조적으로 그렇게 되어야만 하는 것이 아니라, 법선 방향 가속도를 반력($lambda grad Phi$)이 상쇄하는 구조로 식을 세운 후 기본형식과 모양연산자 등을 도입했어야 한다.

== 수정내역

- 05.20
  - $vb(u) times vb(n) approx 1$의 근사를 배제하고 $cos theta$를 통해 값을 정확히 하였습니다. 이에 따라 제약조건이 조정되었습니다.
  - 플람의 포물면에서의 주곡률 계산 오류를 바로잡고, 그에 따라 달라진 물리적 의미를 새롭게 해석하여 결론이 달라졌습니다.
  - 플람의 포물면에서 유도한 공식을 적용 시 생기는 모순이 모순이 아니라 해석의 문제임을 인식하고, 넘어짐 제약 조건을 풀고 다시 계산을 해 새로운 결과를 얻었습니다.
