...
현재까지 상황을 정리해 보자. 접점 $vb(r)(t)$에서의 속도는 $vb(v) := dot(vb(r))$이고, 팽이의 대칭축은 $vb(u)$이다. 

팽이의 대칭축 $vb(u)$와 법선 $vb(n)$이 편각(偏角)을 $theta$라고 한다면 $vb(u) dot vb(n) = cos theta$이다. 축 $vb(u)$를 법선 방향 성분과 접평면 방향 성분 $vb(u)_parallel$로 분리하면 다음과 같다. 이때 분리했으므로 $vb(u)_parallel perp vb(n)$이다.
$
