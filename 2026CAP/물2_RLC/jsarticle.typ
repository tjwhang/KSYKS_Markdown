...
#let font-body = (
  ...
  (name: "Bookk Myungjo", covers: regex("[\p{scx:Hangul}]")),
...
#let cjk-text(it) = {
    ...
    show regex("[\p{scx:Hangul}]++"): it => context {
      ...
      set text(
        baseline: -0.07em, tracking: -0.02em,
        // stroke: 0.005em
      )
      it
    }
...
#let jsarticle-book(
  ...
  show math.equation: set text(font: font-math, stylistic-set: (), weight: "regular")
  show math.equation.where(block: false): it => {
    context {
      let size = measure(it)
      if size.height >= (par.leading/2 + 1em).to-absolute() {
...
#let jsicover(title, subtitle: none, author: none) = [
  ...
  #[
    ...
    #v(25%)
    #align(center)[
      #text(size: 4.2em, font: font-body, weight: "semibold", tracking: 0.1em)[#cjk-text(title)]
      #if subtitle != none [
        #v(-0.3em)
        #text(size: 1.2em, font: font-maru, weight: "medium")[#cjk-text(subtitle)]
      ]
      #if author != none [
        #v(5em)
        #text(size: 1.3em, font: font-maru, weight: "medium")[#cjk-text(author)]
      ]
    ]
    #v(1fr)
  ]
