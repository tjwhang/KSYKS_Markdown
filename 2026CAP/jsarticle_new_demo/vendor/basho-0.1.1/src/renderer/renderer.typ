...
#import "../utils/text.typ": extract-text, upright-features, upright-font
...
#let render-hanging(token, config) = {
  let font = upright-font(token.text, token-font(token, config), config)
  let f-opt = if font != none { (font: font) } else { (:) }
  box(
    width: config.sizing.char-box,
    height: 0pt,
    ...
    align(center + top, text(
      ..f-opt,
      features: upright-features(token.text, config.features),
      token.text,
    )),
  )
}
...
#let render-char-token(token, config) = {
  ...
  let font = token-font(token, config)
  let f-opt = if font != none { (font: font) } else { (:) }
  let heading-font = upright-font(token.at("text", default: none), token.at("font", default: config.at("heading-font", default: none)), config)
