...
#import "../utils/ruby-space.typ": ruby-text-height
...
#let render-ruby(token, config) = {
  let base-space = config.at("space-width", default: 0.25em)
  let reading-space = base-space * (config.sizing.ruby-size / config.sizing.char-box)
  let base-height-for(value) = ruby-text-height(value, config.sizing.char-box, base-space)
  let reading-height-for(value) = ruby-text-height(value, config.sizing.ruby-size, reading-space)
    ...
    let glyphs = stack(
      ...
      let ruby-char = if type(ch) == str {
        text(
          ...
          ..(if upright-font(ch, reading-font, config) != none { (font: upright-font(ch, reading-font, config)) } else { (:) }),
          features: upright-features(ch, config.features),
          ch,
        )
      } else { ch }
      box(
        width: config.sizing.ruby-size,
        height: if ch == " " { reading-space } else { config.sizing.ruby-size },
      ...
      ..token.at("ruby-segments").enumerate().map(((index, pair)) =>
        render-pair(
          ...
          spans.at(index, default: (
            base-height: base-height-for(pair.base),
            reading-height: reading-height-for(pair.ruby),
            span-height: calc.max(base-height-for(pair.base), reading-height-for(pair.ruby)),
          )),
        ),
      ),
    )
  ...
  let span = spans.at(0, default: (
    base-height: base-height-for(token.text),
    reading-height: reading-height-for(token.ruby),
    span-height: calc.max(base-height-for(token.text), reading-height-for(token.ruby)),
  ))
