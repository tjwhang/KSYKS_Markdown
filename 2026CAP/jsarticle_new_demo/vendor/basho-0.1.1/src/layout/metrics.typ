...
#let prepare-layout(tokens, config, usable-height: auto) = {
  ...
  for (index, source) in tokens.enumerate() {
    ...
    if kind in ("newline", "parbreak", "heading-anchor") {
      ...
      let base-space = resolve-y(default-space-width)
      let reading-space = base-space * (ruby-size / char-box)
      let base-height = ruby-text-height(token.text, char-box, base-space)
      ...
      if "ruby-segments" in token {
        for pair in token.at("ruby-segments") {
          let pair-base = ruby-text-height(pair.base, char-box, base-space)
          let pair-reading = ruby-text-height(pair.ruby, ruby-size, reading-space)
        ...
        let reading-height = ruby-text-height(token.ruby, ruby-size, reading-space)
