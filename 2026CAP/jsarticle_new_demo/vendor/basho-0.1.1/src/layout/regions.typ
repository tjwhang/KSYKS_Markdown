...
#let prepare-queues(streams, height) = streams.map(stream => {
  let prepared = prepare-layout(stream.tokens, stream.config, usable-height: height)
  let columns = paginate(
    prepared.tokens,
    height,
    prepared.config,
    paragraph-start: stream.at("paragraph-start", default: true),
  )
  let protect = (
    prepared.config.layout.at("orphan-lines", default: 1) > 1
      or prepared.config.layout.at("widow-lines", default: 1) > 1
  )
  ...
  if protect {
    ...
    for column in columns {
      ...
      for token in column {
        if (not token.at("synthetic", default: false)
            and token.type != "heading-anchor"
            and "source-index" in token) {
...
#let take-cell(queues, cursors, height, width, stream-gap, overhang-threshold,
  render: true, guard-boundary: false, allow-empty-rollback: false) = {
  let selected = ()
  let used = 0pt
  let stopped = false
  ...
  for (stream-index, queue) in queues.enumerate() {
    ...
    while cursor < queue.columns.len() {
      let boundary = if selected.len() == 0 { 0pt } else {
        calc.max(
          if previous-stream == stream-index { queue.line-gap } else { stream-gap },
          queue.ruby-overhangs.at(cursor),
        )
      }
  ...
  if guard-boundary and selected.len() > 0 {
    ...
    if (meta != none and next-meta != none
        and meta.paragraph == next-meta.paragraph) {
      let suffix = 0
      for entry in selected.rev() {
        if (entry.stream-index == last.stream-index
            and entry.line-meta != none
            and entry.line-meta.paragraph == meta.paragraph) {
          suffix += 1
        } else { break }
      }
      ...
      let begins-here = (
        suffix > 0
          and selected.at(selected.len() - suffix).line-meta.index == 0
      )
  ...
  let rendered = if render {
    ...
    for (index, entry) in selected.enumerate() {
      ...
      output.push(box(
        width: entry.width + gap-after,
        height: height,
        align(right + top, render-column(entry.column, entry.config)),
      ))
    }
    output
  } else { () }
  (
    content: if render { text(lang: "und", features: (jsvm: 1), box(
      width: width,
      height: height,
      align(right + top, stack(dir: rtl, spacing: 0pt, ..rendered)),
    )) } else { none },
    cursors: cursors,
  )
}

#let take-grid(queues, cursors, height, width, columns, rows, column-gap,
  row-gap, stream-gap, overhang-threshold,
  render: true, allow-empty-rollback: false) = {
  ...
  for cell-index in range(cell-count) {
    ...
    let cell = take-cell(
      queues, cursors, cell-height, cell-width, stream-gap,
      overhang-threshold,
      render: render,
  ...
  let cells = if render {
    ...
    for row in range(used-rows) {
      ...
      for _ in range(columns - count) { output.push([]) }
      if count > 0 {
        for cell in logical.slice(start, start + count).rev() { output.push(cell) }
      }
    }
    output
  } else { () }
...
#let layout-tate-regions(
  ...
  let minimum-height = calc.max(..active.map(stream => measure(box(
    height: stream.config.sizing.char-box,
  )).height))
  ...
  let first-rows = fitted-row-count(
    height, full-height, rows, row-gap, row-fit-threshold,
  )
  ...
  let first = take-grid(
    first-queues, first-cursors, height, width, columns, first-rows,
    column-gap, row-gap, stream-gap, overhang,
    render: part != "rest",
  ...
  for (index, stream) in active.enumerate() {
    ...
    while consumed < queue.source.len() and queue.source.at(consumed).type in (
      "newline", "parbreak",
    ) {
    ...
    if consumed < queue.source.len() {
      remaining.push((
        tokens: queue.source.slice(consumed),
  ...
  if remaining.len() > 0 {
    ...
    while not queues-done(queues, cursors) {
      let page = take-grid(
        queues, cursors, full-height, width, columns, rows,
        column-gap, row-gap, stream-gap, overhang,
      )
