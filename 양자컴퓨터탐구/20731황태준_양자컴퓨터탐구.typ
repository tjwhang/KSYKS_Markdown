...
#definition(title: "벡터의 텐서곱")[
  ...
  $
    ket(phi) times.o ket(psi) = sum_((a,b) in Sigma times Gamma) alpha_a Beta_b ket(a b)
  $

  이 새로운 벡터의 원소들은 $Sigma times Gamma$의 원소들에 대응된다. 또, 이 정의는 $ket(pi) := ket(phi) times.o ket(psi)$에 대해 아래와 동치이다.
  $
...
#theorem(title: "텐서곱을 이용한 계의 독립 조건")[
  ...
  $
    ket(pi) = ket(phi) times.o ket(psi) wide "where " ket(pi) in RR^(abs(Sigma) times abs(Gamma))
  $
]

텐서곱은 이러한 정보 처리에서 자주 쓰이고 중요하기 때문에, 아래와 같은 표기로 기호가 생략 및 간략화되기도 한다.
$
  ket(phi) times ket(psi) equiv ket(phi) ket(psi) equiv ket(phi times.o psi) equiv ket(phi psi) equiv ket((phi, psi)) equiv ket(phi\, psi)
$

또, 텐서곱 연산에서는 분배 법칙과 스칼라에 한해 결합법칙이 성립한다.
$
  (ket(phi_1) + ket(phi_2)) times.o ket(psi) = ket(phi_1) times.o ket(psi) + ket(phi_2) times.o ket(psi)
$
$
  & (alpha ket(phi)) times.o ket(phi) = ket(phi) times.o (alpha ket(psi)) = alpha(ket(phi) times.o ket(psi)) \
...
이제 $sX = a$인 사건에 대해 계 $(sX, sY)$에서 해당 확률은 $ket(a) times.o ket(pi_a)$가 된다.
...
$
  ket(psi) = ket(0) times.o (1/2 ket(1) + 1/12 ket(3)) + ket(1) times.o (1/12 ket(1) + 1/6 ket(2) + 1/6 ket(3))
$
