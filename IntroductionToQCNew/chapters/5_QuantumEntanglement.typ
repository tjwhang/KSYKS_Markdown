...
#definition(title: [텐서곱])[
  $U$를 기저 ${vb(u)_1, ..., vb(u)_n}$을 갖는 벡터공간, $V$를 기저 ${vb(v)_1, ..., vb(v)_k}$를 갖는 벡터공간이라고 하자. 이때 텐서곱 $U times.o V$는
  $
    U times.o V = Set(vb(u)_i times.o vb(v)_j, i = 1\, ...\, n and j = 1\,...\,k)
  $

  이때 $dim(U times.o V) = dim U times dim V$이다.
]

$times.o$ 기호는 $vb(u)$와 $vb(v)$를 엮어 합친 새로운 벡터를 만드는 역할 정도로 보면 된다. 수학적으로는 다음과 같이 정의된다.
$
  times.o : U times V -> U times.o V
$
즉 $vb(u) in U, vb(v) in V$일 때
$
  (vb(u), vb(v)) mapsto vb(u) times.o vb(v)
$

이때 $times.o$은 쌍선형으로, 쌍선형곱
$
  (sum_(i = 1)^n a_i vb(u)_i) times.o (sum_(j=1)^k b_j vb(v)_j) = sum_(i = 1)^n sum_(j = 1)^k a_i b_i (vb(u)_i times.o vb(v)_j)
$
이 성립한다.
...
#theorem(title: [상호작용하는 두 양자계의 합성 상태공간])[
  상태공간으로 각각 $U, V$를 가지는 두 양자계가 있을 때, 두 계의 상태공간을 합성하여 하나로 본 상태공간은 $U times.o V$이다.
]

이때 이 새로운 상태공간 $U times.o V$에서의 내적은
$
  braket(vb(u)_1 times.o vb(v)_1, vb(u)_2 times.o vb(v)_2) = braket(vb(u)_1, vb(u)_2) dot braket(vb(v)_1, vb(v)_2)
$

이제 이것을 가지고 $n$-큐비트를 생각해 보자. 먼저, 단일 큐비트의 경우는 큐비트 상태공간 $B = CC^2$을 가지고, 기저는 ${ket(0), ket(1)}$이다. 2-큐비트는 상태공간 $B times.o B$를 가지고 기저는 ${ket(0) times.o ket(0), ket(0) times.o ket(1), ket(1) times.o ket(0), ket(1) times.o ket(1)}$이다.

우리는 앞서 다주 큐비트를 간단히 줄여 나타내는 방법을 보았었다. 2-큐비트의 기저를 이 방법으로 표현하면 ${ket(00), ket(01), ket(10), ket(11)}$이 된다. 이 표현법에는 아래와 같은 정의가 내포되어 있다.
$
  ket(a_1) times.o ... times.o ket(a_n) = ket(a_1 ... a_n)
$

$n$-큐비트는 상태공간으로 $B_n = underbrace(B times.o B times.o ... times.o B, n)$, 기저로는 ${overbrace(ket(00...0), n), ..., overbrace(ket(11...1), n)}$를 갖는다. 이때 $dim B_n = 2^n$이다.

예를 들어, 서로 상호작용하지 않는 두 입자가 각각 상태 $psi_1 = a ket(0) + b ket(1)$과 $psi_2 = c ket(0) + d ket(1)$을 가질 때, 합성된 상태는
$
  psi_1 times.o psi_2 & = (a ket(0) + b ket(1)) times.o (c ket(0) + d ket(1)) \
                           & = a c ket(00) + a d ket(01) + b c ket(1 0) b d ket(11)
$

#problem[
  임의의 2-큐비트 상태는 $psi_1 times.o psi_2$ 꼴로 분해될 수 있는가?
]

#solution[
  없다. $z_1 ket(00) + z_2 ket(01) + z_3 ket(10) + z_4 ket(11) = psi_1 times.o psi_2$이려면 $z_2 z_3 = z_1 z_4$이어야 하는데, 항상 그런 것은 아니기 때문이다. 예를 들어 흔하다고 할 수 있는 상태 $psi = 1/sqrt(2) ket(00) + 1/sqrt(2) ket(11)$은 $psi_1 times.o psi_2$로 표현될 수 없다.
]

== 양자 얽힘의 정의

#definition(title: [양자 얽힘])[
  어떤 양자계가 각각 상태공간 $U, V$를 가지는 각각의 두 부분으로 되어 있다고 하자. 이때 이 다중 양자계의 상태 $phi in U times.o V$는 $psi_1 in U, psi_2 in V$에 대해 $phi != psi_1 times.o psi_2$이면 얽혀 있다고 한다.
]
...
#problem[
  $psi_1, psi_2$에 대해 얽힘이 없는 합성 큐비트 상태 $psi_1 times.o psi_2$는 관측 사건이 서로 독립임을 보여라.
]

#solution[
  $
    psi &= (a ket(0) + b ket(1)) times.o (c ket(0) + d ket(1)) \
...
이것은 아인슈타인이 틀린 말을 한 몇 안 되는 사례이다. 이 으스스한 원격 작용이라는 것이 여러 실험을 통해 확인되었기 때문이다. 그 중 가장 대표적인 실험은 2017년 중국이 양자 통신 위성 묵자(墨子, Micius)을 발사하여 진행한 것이다. 위성은 우주에서 1200 km 이상 떨어진 지구의 두 관측소를 향해 각각 광자를 발사한 결과 거리가 멀어도 이러한 현상이 일어나는 것을 증명했다. 
...
$
  psi = (a_1 ket(0) + b_1 ket(1)) times.o ... times.o (a_n ket(0) + b_n ket(1))
$
